...
def findKNearestNeighbors(testSample, k):
	...
	for i in range(lenTrainingDataSet):
		d = calDistance(2, trainingData[i], testSample)
...
def knn(datasetDir, trainingFileName, testingFileName, k, best_predictors):
	#datasetDir = "D:/CD/FA16/AML/datasets/wine quality/knn/"
	...
	# print(testingData)
	# input()
	#readData(datasetDir+"testingDataNorm.csv",testingData)
	predictions = []
	i=1
	...
	computeAccuracy(testingData, predictions)